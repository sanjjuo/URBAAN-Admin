        ...
        <>
            {addedCategories.map((item) => (
                <div className="bg-white rounded-xl shadow-md p-5 h-64 space-y-6 relative" key={item.id}>
                    ...
                    {/* Subcategories List */}
                    <ul className="space-y-2 text-secondary/50 text-xs">
                    ...
                    {/* Read More Link */}
                    <Link to="#" onClick={()=>handleOpen("readMoreModal")} className="absolute bottom-5 text-buttonBg text-sm flex items-center gap-2 hover:text-blue-900">
                        Read more <FaArrowRight />
                    </Link>
                </div>
            ))}

            <DeleteModal open={open === "deleteModal"} handleOpen={handleOpen}/>
            <ReadMoreModal open={open === "readMoreModal"} handleOpen={handleOpen}/>
        </>
    );
};
